...
def run_leakage_free_tuning(
    ...
    NOW SUPPORTS consensus gene filtering for Chapter 3.
    ...
    # ========================================================================
    # NEW: Filter to consensus genes if configured (Chapter 3)
    # ========================================================================
    if config['data'].get('use_consensus_genes', False):
        ...
        try:
            ...
            logger.error("To create consensus gene file, run: Rscript scripts/save_consensus_genes.R")
    else:
    ...
    # Save summary (with consensus gene info)
    summary = {
        'cohort': cohort,
        'n_samples': expr_raw.shape[1],
        'n_genes': expr_raw.shape[0],  # Current gene count
        'use_consensus_genes': config['data'].get('use_consensus_genes', False),
