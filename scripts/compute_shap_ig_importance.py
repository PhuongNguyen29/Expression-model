...
def compare_importance_methods(
    ...
    # Top-50 agreement
    ig_top50 = set(np.array(gene_names)[np.argsort(-ig_importance)[:50]])
    l2_top50 = set(np.array(gene_names)[np.argsort(-l2_importance)[:50]])
    ...
    if shap_results is not None:
        shap_top50 = set(np.array(gene_names)[np.argsort(-shap_importance)[:50]])
